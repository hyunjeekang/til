...
회원 가입 폼에서 POST로 데이터를 제출하고, 처리 후 Forward로 결과 페이지를 보여주면:
 
```
POST /member?action=member-regist
  → INSERT 실행
  → Forward → 결과 JSP 렌더링
```
 
이 상태에서 새로고침하면 브라우저는 마지막 요청인 `POST /member?action=member-regist` 를 재전송한다. INSERT가 다시 실행되어 중복 데이터가 생긴다.
 
### 해결: PRG 패턴
 
POST 처리 완료 후 Redirect로 GET 요청을 유도한다. 이후 새로고침은 마지막 GET 요청만 반복하므로 중복 제출이 발생하지 않는다.
...
### alertMsg를 session에 담는 이유
 
POST → Redirect → GET 흐름에서 `request` 객체는 Redirect 시점에 초기화된다. "등록되었습니다" 같은 알림 메시지를 다음 페이지에 전달하려면 `session`에 담아야 한다.
...
// GET으로 도착한 index.jsp에서
// session에서 alertMsg를 꺼내 alert으로 표시하고 session에서 제거한다
```
 
`request.setAttribute()` 로 담으면 Redirect 후 소멸하므로 반드시 `session.setAttribute()` 를 사용한다.
 
### GET, PUT, DELETE는 왜 괜찮을까?
 
이 메서드들은 **멱등(idempotent)** 하다. 같은 요청을 여러 번 보내도 결과가 동일하다. POST는 멱등하지 않아서 반복 실행 시 결과가 달라지므로 PRG 패턴이 필요하다.

<br>

## 5. JSP 호출 방법과 WEB-INF 보안
